...
def generate_subtitle_worker(args):
    """工作进程：为单个视频生成字幕"""
    video_file, model_name, device, worker_id = args
    import whisper
    
    base_name = os.path.splitext(video_file)[0]
    srt_file = f"{base_name}_base.srt"
...
def main():
    ...
    for f in os.listdir('.'):
        if f.lower().endswith(supported_formats):
            ...
            # 检查是否已有字幕
            if not any(os.path.exists(f"{base}{suffix}") for suffix in ['_base.srt', '_small.srt', '.srt']):
    ...
    print(f"找到 {len(videos_to_process)} 个需要生成字幕的视频")
    ...
    # 准备任务参数
    tasks = [(video, model_name, device, i % num_workers) 
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        # 提交所有任务
        future_to_video = {executor.submit(generate_subtitle_worker, task): task[0] 
                          for task in tasks}
        
        # 使用 tqdm 显示进度
        with tqdm(total=len(videos_to_process), desc="处理进度") as pbar:
            for future in as_completed(future_to_video):
                ...
                try:
                    ...
                    if success:
                        ...
                        failed_videos.append((video, error))
                except Exception as e:
                    failed_videos.append((video_file, str(e)))
    ...
    print(f"\n处理完成！")
    print(f"成功: {success_count}/{len(videos_to_process)}")
    print(f"总用时: {elapsed_time:.1f} 秒")
    print(f"平均速度: {elapsed_time/len(videos_to_process):.1f} 秒/视频")
    ...
    if len(videos_to_process) > 1:
        single_thread_estimate = elapsed_time * num_workers / len(videos_to_process) * len(videos_to_process)
